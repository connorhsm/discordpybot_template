...
class Error_Handling(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            ...
            await ctx.message.delete()
            await ctx.send(f'{ctx.author.mention}, Missing argument. See {await self.template_bot.get_prefix(ctx)}help', delete_after=10)
            ...
            await ctx.message.delete()
            await ctx.send(f'{ctx.author.mention} Invalid input. See {await self.template_bot.get_prefix(ctx)}help', delete_after=10)
...
def setup(template_bot):
    template_bot.add_cog(Error_Handling(template_bot))